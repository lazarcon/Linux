...
#############################################################################################################
#
# Starts the Backup Server, if not already running
#
# Param1: server hostname to ping
# Param2: mac address of host to ping
#############################################################################################################
...
if [ $? -ne 0 ]
then
  echo "$1 not running. Trying to start (this will take about 3 Minutes)"
  wakeonlan $2
  sleep 150
  ping -c $RETRYCOUNT $1 > /dev/null 2>&1
  if [ $? -ne 0 ]
  then
    ...
    echo "Try to start $1 manually and run the script again"
    exit 1
  fi
else
