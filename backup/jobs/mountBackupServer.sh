...
#############################################################################################################
#
# Mounts the Backup Server, if not already running
#
# Param1: server hostname to ping
# Param2: path to mount
#############################################################################################################
# Retry count of ping request
echo "Mounting QNAP to $2 ..."
rm -rf $2 >/dev/null
mkdir $2
#mount $1:/cola $2 -o vers=3,user=cola > /dev/null 2>&1
mount -t nfs $1:/cola $2
if [ $? -ne 0 ]
then
  echo "Could not mount $1 to $2"
  exit 1
else
  echo " ... done"
fi
